...
function getIdFromPath(path = '') {
  const parts = path.split('/posts/metrics/');
  if (parts.length < 2) {
    return null;
  }
...
exports.handler = async (event) => {
  try {
    ...
    const posts = await supabaseRequest(`post_payloads?id=eq.${id}&limit=1`);
    const post = posts[0];
    if (!post) {
      return { statusCode: 404, body: 'Not found' };
    }

    await ensureRole(post.brand_id, userId, ['owner', 'editor', 'viewer']);

    const metrics = await supabaseRequest(`post_metrics?post_id=eq.${id}`);

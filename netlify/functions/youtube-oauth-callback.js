...
async function exchangeToken(code, redirectUri) {
  ...
  if (!response.ok || payload.error) {
    throw new Error(payload.error_description || 'YouTube token exchange failed');
  }
  return payload;
}
...
async function fetchYoutubeChannel(accessToken) {
  ...
  const payload = await response.json();
  if (!response.ok) {
    throw new Error(payload.error?.message || 'YouTube channel fetch failed');
  }
  const channel = payload.items?.[0];
  return channel
    ? { id: channel.id, title: channel.snippet?.title || channel.id }
    : null;
}
...
exports.handler = async (event) => {
  try {
    ...
    const redirectUri = `${baseUrl}/youtube/oauth/callback`;
    ...
    await supabaseRequest('network_connections?on_conflict=brand_id,network,account_id', {
      ...
      body: {
        ...
        user_id: userId || null,
        network: 'youtube',
        account_id: channel?.id || brandId,
        account_name: channel?.title || 'YouTube',
        access_token: accessToken,
        refresh_token: refreshToken,
        token_expires_at: tokenExpiresAt,
        scopes: ['youtube.upload'],
        meta_user_id: channel?.id || null,
        is_active: !atLimit,
    ...
    return {
      statusCode: 302,
      headers: {
        Location: `${safeRedirect}?oauth=${atLimit ? 'limit' : 'success'}&oauth_network=youtube`
      }
    };
  } catch (error) {
    return {
      statusCode: 302,
      headers: { Location: '/editor.html?oauth=error&oauth_network=youtube' }
    };
  }
};

...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  if (!session) {
    ...
    if (checkoutBtn) {
      ...
      checkoutBtn.textContent = getI18n('checkout.loginBtn', 'Please log in to continue');
    }
    return;
  }
...
async function initializeCheckout(user) {
  ...
  const plans = {
    creator: {
      ...
      featureKeys: [
        'checkout.features.brands',
        'checkout.features.analytics',
        'checkout.features.ai'
      ]
    },
    business: {
      name: 'Business',
      ...
      featureKeys: [
        'features.business.brands',
        'features.business.reports',
        'features.business.inbox'
      ]
    },
    agency: {
      name: 'Agency',
      ...
      featureKeys: [
        'features.agency.brands',
        'features.agency.reports',
        'features.agency.approvals'
      ]
    }
  };
  ...
  const features = selectedPlan.featureKeys.map((key) => getI18n(key, key));
  ...
  paymentSection.innerHTML = `
    <h3 class="checkout-subtitle">${getI18n('checkout.paymentTitle', 'Payment Method')}</h3>
    <p class="checkout-note">${getI18n('checkout.secureNote', 'Click below to continue with Stripe')}</p>
    <button type="button" id="checkoutBtn" class="btn btn--primary btn--full">
      ${getI18n('checkout.cta', 'Start Free Trial')}
    </button>
  `;
  ...
  newCheckoutBtn.addEventListener('click', async function() {
    this.disabled = true;
    this.textContent = getI18n('checkout.processing', 'Processing...');
    ...
    try {
      ...
      this.disabled = false;
      this.textContent = getI18n('checkout.cta', 'Start Free Trial');
